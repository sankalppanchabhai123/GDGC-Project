  ...
  return (
    ...
    >
      ...
      <div className={classes.eventsContainer}>
        <div className={classes.upComingEventsContainer}>
          ...
          <div className={classes.cards}>
            {upcomingEvents &&
              upcomingEvents?.map((current, idx) => (
                <EventCard key={idx} current={current} />
              ))}
            {upcomingEvents.length === 0 && (
              <h1 className={classes.noEventInfo}>
                No upcoming Events Right Now But we will be back shortly
              </h1>
            )}
          </div>
        </div>
        <div className={classes.prevEventsContainer}>
          ...
          <div className={classes.cards}>
            {previousEvents?.map((current, idx) => (
              <EventCard key={idx} current={current} />
            ))}
      ...
      <Button
        ...
        onClick={() => {
          window.open(
            "https://gdg.community.dev/gdg-on-campus-dy-patil-college-of-engineering-pune-india/",
            "_blank"
          );
        }}
      />
